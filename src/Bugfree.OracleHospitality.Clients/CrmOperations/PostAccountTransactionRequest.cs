...
{
    // DESIGN: this multi-purpose operation has the ability to create a loyalty
    // account as well. We could use it in place of the SV_POINT_ISSUE
    // operation, but then we'd have to infer program code, length of preamble,
    // and more that SV_POINT_ISSUE infers from account number. On the upside,
    // this operation doesn't require correct POS setup, (probably) doesn't
    // leave a trace in the account's transaction log, and (probably) isn't
    // subject to the 9,999 check number limit without grace period that limits
    // SV_POINT_ISSUE.
    //
    ...
    // of POS API elements just because names of element overlap. With CRM API,
    // for instance, documentation for TraceID characterizes it as "[a]n
    // arbitrary trace number, used to specifically identify a message." whereas
    // POS API equivalent mandates a strict format.
